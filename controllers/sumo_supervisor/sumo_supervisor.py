...
try:
    ...
    if 'SUMO_HOME' in os.environ:
        ...
        # Try to auto-detect SUMO on macOS (Homebrew)
            ...
            for basePath in possiblePaths:
                if os.path.exists(basePath):
                    ...
                    if os.path.islink(basePath):
                        sumoPath = os.path.realpath(basePath)
                    else:
                        sumoPath = basePath
                    print('Auto-detected SUMO from %s' % sumoPath)
                    break
            if not sumoPath:
                sumoImportError()
        else:
    ...
    # Try standard location first, then Homebrew location
    toolsPath = os.path.join(sumoPath, 'tools')
    if not os.path.exists(toolsPath):
        toolsPath = os.path.join(sumoPath, 'share', 'sumo', 'tools')
